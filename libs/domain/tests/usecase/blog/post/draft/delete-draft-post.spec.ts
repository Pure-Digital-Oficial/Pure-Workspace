...
interface SutTypes {
  sut: DeleteDraftPost;
  deletePostDto: DeleteDraftPostDto;
  findUserByIdRepository: FindUserByIdRepository;
  findPostByIdRepository: FindPostByIdRepository;
  deleteDraftPostRepository: DeleteDraftPostRepository;
}

const makeSut = (): SutTypes => {
  const findUserByIdRepository = new FindUserByIdRepositoryMock();
  const findPostByIdRepository = new FindPostByIdRepositoryMock();
  const deleteDraftPostRepository = new DeleteDraftPostRepositoryMock();

  const deletePostDto: DeleteDraftPostDto = {
    id: PostMock.id,
    loggedUserId: userMock.userId,
  };

  const sut = new DeleteDraftPost(
    findUserByIdRepository,
    findPostByIdRepository,
    deleteDraftPostRepository
  );
...
describe('DeleteDraftPost', () => {
  it('should return Post ID when pass correct deletePostDto', async () => {
  ...
  it('should return EntityNotDeleted when a not delete Post in system', async () => {
    const { deletePostDto, sut } = makeSut();
    jest
      .spyOn(sut['deleteDraftPostRepository'], 'delete')
