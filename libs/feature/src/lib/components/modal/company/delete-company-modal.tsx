...
import { DeleteCompanyByIdDto, ErrorResponse } from '@pure-workspace/domain';
...
export const DeleteCompanyModal: FC<DeleteCompanyModalProps> = ({
  handlePopUpClose,
  showAlert,
  open,
  title,
  ...
  buttonNoTitle = 'Não',
  buttonYesTitle = 'Sim',
  subTitle = 'Caso você deseje realmente deletar a empresa por favor indicar o motivo no campo a baixo:',
}) => {
  ...
  const deleteCompany = async (request: DeleteCompanyByIdDto) => {
    try {
      const result = await DeleteCompanyByIdRequest(request);
      return result;
    } catch (error) {
      console.error(error);
