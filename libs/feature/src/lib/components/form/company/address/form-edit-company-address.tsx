...
export const FormEditCompanyAddress: FC<FormEditCompanyAddressProps> = ({
  ...
  const getCompanyAddress = useCallback(
    async (input: FindCompanyAddressByIdDto) => {
      try {
        const result = await FindCompanyAddressByIdRequest(input);
        setDataLoaded(true);
        return result;
      } catch (error) {
        console.error(error);
  ...
  const editCompanyAddress = async (input: EditCompanyAddressDto) => {
    try {
      ...
      const result = await EditCompanyAddressRequest(input);
      return result;
    } catch (error) {
      setLoading(false);
      setSuccess(false);
